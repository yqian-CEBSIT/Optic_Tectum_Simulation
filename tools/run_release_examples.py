...
def main() -> None:
    ...
    (FIGURE3_RESULTS / "release_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
    (FIGURE5_RESULTS / "release_manifest.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False))
    print("[done] all release example jobs", flush=True)
